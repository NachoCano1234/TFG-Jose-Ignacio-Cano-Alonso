...
class AIPlayer:
    def __init__(self, state_dim, action_space_dim, save_dir, date, config: Config):
        ...
        if torch.cuda.is_available():
            #self.device = "cuda"
            self.device = "cpu" #Cambio por Nacho para forzar el uso de CPU
    ...
    def act(self, state):
        ...
        # EXPLOIT
            ...
            with torch.no_grad(): 
                neuralNetOutput = self.net(state, model="online") #Cambio por Nacho para obtener la salida de la red neuronal utilizando el modelo "online".
    ...
    def cache(self, state, next_state, action, reward, done):
        ...
        self.memory.append((state, next_state, action, reward, done)) #Cambio por Nacho para almacenar los estados como arrays de numpy en lugar de tensores
